...
###########################################################################################
	...
	def remove_housekeepingenes(self, adata, path=None):
		...
		"""
		if path is None:
			print("path must be passed as list")
			return
		if path == []:
			print("empty list")
			return
		...
		hkg = []
		for i in path:
			hkg.append(np.loadtxt(i, dtype="str"))
	...
	def remove_features_pct(self, adata, group_by=None, pct=0.3):
		"""\
			Removing genes expressed in less than pct within group_by from AnnData object.
			...
			-------
			adata : anndata.AnnData
				The AnnData without low expressed genes within group_by.
		"""
	...
	def remove_features_pct_2groups(self, adata, group_by=None, pct1=0.9, pct2=0.5):
		"""\
			Removing highly expressed genes within group_by from AnnData object.
			...
			pct2 : flaot (default: 0.5)
				Total pct of group_by groups 
                
			Returns
			-------
			adata : anndata.AnnData
				The AnnData without highly expressed genes.
		"""
		# raw.X should be log trasformed
		if group_by is None:
			print("select a group_by in .obs")
	...
	def scale_filter_features(self, adata, n_components=None, filter_expression=True):
		"""\
			The filtered matrix of cells and genes is then dimensionally-reduced using principal component analysis (PCA) [15] applied to each gene as a unique feature. The PCA is then inverted and the top 20 genes in each principal component explaining up to 80 % of the overall variance in the data are retained. This additional feature guards against genes which would add very little extra information about the variance of expression profiles in the data and speeds up subsequent training of the model.
			...
			-------
			adata : anndata.AnnData
				The AnnData with filtered genes.
		"""
		X = adata.X
	...
	def DNN(self, adata, group_by=None, model=None, test_size=0.2, balance=True, verbose=True, save=True):
		"""\
			...
			model : Keras model (default: None)
				DNN model, if None smashpy uses the default one
	...
	def run_shap(self, adata, group_by=None, model=None, verbose=True, pct=0.05, restrict_top=("global", 10)):
		"""\
			...
			pct : float (default: 0.05)
				pct of test on what to run shap
			restrict_top : set(string, integer) (default: ("global", 10))
				restrict the search to all groups of for each group defined by group_by
...
# 				clf = XGBClassifier(max_depth=9, num_class=n, n_estimators=200, objective="binary:logistic", learning_rate=0.25, booster="dart", random_state=SEED)
	...
	def run_classifiers(self, adata, group_by=None, genes=None, classifier="KNN", balance=False, title=None, save=True):
		"""\
			Test the results obtained by shapley and gini importance by using machine learning appraoches.
			...
			genes : list (default: None)
				returned by run_shap or gini_importance
			classifier : sklearn.classifier (default: KNN)
				clf can be SVM, RF, or KNN
	...
	###########################################################################################
	def sort_and_plot(self, adata, selectedGenes, group_by=None, top=5, figsize=(5,8), restricted=True):
		"""\
			Sorting and plotting the result returned by run_shap or gini_importance dictionaries or lists.
			...
			selectedGenes : list (default: None)
				list/dict returned by run_shap and gini_importance 
			group_by : string (default: None)
				AnnData column				
			top : integer (default: 5)
				Number of genes to show for each group_by
			figsize : set(float, float) (default: (5,8))
 			restricted : boolean (default: True)
				If true it removes genes expressed in less than 30% within group_by
                
			Returns
			-------
			ax : Figure axes
				...
			finalSelectedGenes_dict_top : dict
		"""
		if group_by is None:
			print("select a group_by in .obs")
			return
